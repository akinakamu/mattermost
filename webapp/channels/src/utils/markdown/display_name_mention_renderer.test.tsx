...
describe('DisplayNameMentionRenderer', () => {
    ...
    testCases.forEach((testCase) => {
        it(testCase.description, () => {
            ...
            // For simpler test comparison, we'll check if the display name is present
            // The exact HTML output may vary slightly depending on the markdown parser
            if (testCase.description.includes('code block')) {
                // Code blocks should preserve the original username
                expect(result).toContain('@john.doe');
                expect(result).not.toContain('@Johnny');
            } else if (testCase.description.includes('inline code')) {
                // Inline code should preserve the original username
                expect(result).toContain('@john.doe');
            } else if (testCase.description.includes('unknown')) {
                // Unknown users should remain unchanged
                expect(result).toContain('@unknown.user');
            } else {
                // The basic assertion for most cases
